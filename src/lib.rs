...
impl<T: CustomProcedure> sys::Guest for T {
	fn run(params: sys::Json) -> Result<sys::Json, sys::RpcError> {
		...
		let params: T::Params = match serde_json::from_slice(&params) {
			Ok(v) => v,
			Err(err) => {
				return Err(sys::rockawayx::rockrpc::types::RpcError {
		...
		match poll_await(Self::run(params)) {
			Ok(v) => match serde_json::to_vec(&v) {
				Ok(v) => Ok(v),
				Err(err) => Err(sys::rockawayx::rockrpc::types::RpcError {
			...
			Err(RpcError {
				...
				Err(sys::rockawayx::rockrpc::types::RpcError {
					code,
					message,
					data,
				})
...
pub struct Subscription(sys::rockawayx::rockrpc::rockrpc_host::RpcSubscription);
impl Subscription {
